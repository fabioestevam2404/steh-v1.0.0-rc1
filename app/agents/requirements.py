...
class RequirementsAgent:
    ...
    def run(self, request: str) -> AgentResult:
        if self.mode == "openai":
            ...
            artifact = RequirementsResult.model_validate(structured.invoke([
                ("system","You are STEH Requirements Agent. Return only structured requirements. Identify functional/non-functional requirements, acceptance criteria, assumptions and open questions."),
                ("human",request),
            ]))
            confidence = 0.90
        else:
            artifact = RequirementsResult(
                ...
                acceptance_criteria=["Fluxo principal executável.", "Entradas inválidas rejeitadas."],
        ...
        return AgentResult(
            ...
            evidence=[{"type":"requirements_artifact","timestamp":datetime.now(timezone.utc).isoformat()}],
