...
class ContainerProcessRunner:
    ALLOWED_SCANNERS = {"semgrep", "gitleaks", "trivy"}
    ...
    def run_scanner(
        ...
        scanner_args = {
            "semgrep": ["semgrep", "scan", "--config", "auto", "--json", "/workspace"],
            "gitleaks": ["gitleaks", "detect", "--source", "/workspace", "--report-format", "json", "--report-path", "/tmp/gitleaks.json", "--no-git"],
            "trivy": ["trivy", "fs", "--format", "json", "--scanners", "vuln,secret,misconfig", "/workspace"],
        }[scanner]
        ...
        try:
            ...
            return ProcessResult(
                ...
                stderr=exc.stderr or "scanner timeout",
